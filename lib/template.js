...
_.template = function(text, settings, oldSettings) {
    ...
    console.log(settings); // 形式参数设置了，但是执行函数的时候并没有传入相应的参数，输出的就是undefined
    console.log(!settings) // 对于undefined来说用!取反得到的是true 这里是一个很好的例子,这个技巧在很多的地方都可以用。

    // 如果settings不存在,并且oldsettings存在 将oldsettings赋值给settings
    ...
    // 这里的templateSettings的 这个对象每一个key对应的是一个正则对象。
    settings = _.defaults({}, settings, _.templateSettings);
    console.log(settings); // {evaluate: /<%([\s\S]+?)%>/g, interpolate: /<%=([\s\S]+?)%>/g, escape: /<%-([\s\S]+?)%>/g}

    // 这里的正则表达式对象当做普通的函数使用,
    // 第一个参数是 String类型 指定的正则表达式模式字符串。
    // 第二个参数是 可选/String类型指定的模式标志，可以为"g"、"i"、"m"之间的任意组合。
    // 这里传入的第一个参数是数组,使用join方法分割成指定的类型字符串 第二个参数是 模式标志这里的模式标志是 'g';
    // RegExp()函数的返回值是RegExp类型，返回一个新的RegExp对象。也就是说这里 matcher 依然是一个正则表达式对象。
    // 正则表达式的source方法,:settings.escape settings.interpolate settings.evaluate 本身就是一个正则表达式可以使用source方法
    // JavaScript RegExp.source 属性详解:
    // source属性用于返回正则表达式模式的文本内容(不包括标志)。
    // 该属性是RegExp对象(实例)的一个只读属性。
